...
public class Agent : ScheduleManager
{
    [Header("Dependencies")]
    [SerializeField] BaseModel model;
    ...
    [SerializeField] bool enabledRandomAction = false;

    [Header("Movement")]
    [SerializeField] float targetMargin = 1f;
    [SerializeField] float maxAllowedFarDistance = 15f;
    [SerializeField] bool enabledMovement = true;
    List<Vector3> path;
    AgentAction goBack;
    ...
    public bool EnabledRandomAction {
        get { return enabledRandomAction; }
        set {
            if( enabledRandomAction != value)
            {
                if(!value)
                {
                    Scheduler.Cancel("ExecuteRandomAction");
                }
                enabledRandomAction = value;
            }
        }
    }
    ...
    {
        get { return enabledMovement; }
        set
        {
            if (value && !enabledMovement) { if (path.Count > 0) Goto(path[0]); }
    ...
    protected override void Update()
    {
        ...
        else if(EnabledRandomAction && !Scheduler.IsActive("ExecuteRandomAction") && !Scheduler.IsActive("ExecuteAction"))
        {
